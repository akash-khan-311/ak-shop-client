...
const BlogItem = ({ blog }: { blog: BlogItem }) => {
  return (
    <div className="shadow-1 bg-white dark:bg-dark rounded-xl px-4 sm:px-5 pt-5 pb-4">
      <Link
      ...
      <div className="mt-5.5">
        <span className="flex items-center gap-3 mb-2.5">
          <Link
            href={`/blog/${blog.id.toString()}`}
            className="text-custom-sm ease-out duration-200 hover:text-pink"
          >
            {blog.date}
          </Link>
        ...
        <h2 className="font-medium text-dark dark:text-white text-lg sm:text-xl ease-out duration-200 mb-4 hover:text-pink dark:hover:text-pink">
          <Link href={`/blog/${blog.id.toString()}`}>{blog.title}</Link>
        </h2>

        <Link
          href={`/blog/${blog.id.toString()}`}
          className="text-custom-sm inline-flex items-center gap-2 py-2 ease-out duration-200 hover:text-pink"
        >
          Read More
          <svg
